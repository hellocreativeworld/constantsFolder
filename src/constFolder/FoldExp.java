...
public class FoldExp implements Exp.Visitor<String, String>{
	...
	public String visit(EApp p, String arg) {
		...
		if(p.listexp_.size()==0){
			...
			String s="";
			for(int index = 0; index < p.listexp_.size(); index++){
				k.add( ConstFolder.eval(p.listexp_.get(index)));
			}
	...
	public String visit(EPIncr p, String arg) {
		...
		String id = ConstFolder.eval(p.exp_);
	...
	public String visit(ETimes p, String arg) {
		...
		Module.Output.add(s1);
		String e1 = ConstFolder.eval(p.exp_1);
		String e2 = ConstFolder.eval(p.exp_2);
